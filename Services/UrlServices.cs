...
public class UrlServices(DbConfig context) : IUrlServices
{
    ...
    public async Task<URL> GetUrl(string email)
    {
        ...
        URL? url = await context.Urls.Include(u => u.Clicks).FirstOrDefaultAsync(u => u.UserId == user.Id);
        if (url is null)
        {
            throw new NotFoundException("Url", email);
        }
